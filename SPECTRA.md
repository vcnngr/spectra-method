...
```
_spectra/
├── core/          Engagement framework, War Room, init, help, Specter, Chronicle, Referee
├── rtk/ 🔴        Red Team Kit — 7 offensive agents (Viper, Ghost, Razor, Phantom, Mirage, Blade, Forge)
├── soc/ 🔵        Security Operations — 8 defensive agents (Commander, Watchdog, Tracker, Hawk, Sentinel, Shield, Keystone, Signal)
├── irt/ 🟠        Incident Response — 6 reactive agents (Dispatch, Trace, Scalpel, Oracle, Surge, Stratus)
├── grc/ ⚪        Governance, Risk & Compliance — 4 strategic agents (Arbiter, Auditor, Scribe, Counsel)
└── ot/ ⚙️         OT/ICS Security — 1 assessment agent (Relay)
```
...
- Module definitions: rtk/module.yaml, soc/module.yaml, irt/module.yaml, grc/module.yaml, ot/module.yaml (interactive config questions)
...
- Execution scripts: scope-enforcer.py (scope verification), blue-live-adapter.py (read-only Blue telemetry ingest), red-blue-broker.py (offline Red/Blue ledger exchange), duel-orchestrator.py (Duel Mode role ledgers and scoring), engagement-state.py (deterministic workflow gates/state transitions), party-orchestrator.py (Party Mode sub-agent planner), report-adapters.py (normalized reporting data adapters), report-generator.py (structured report assembly), evidence-logger.py (chain of custody), tools-registry.yaml (44 tools in 9 categories)
- Engagement schemas: core/schemas/engagement.schema.json and engagement.schema.yaml
- Compact skill index: _config/skill-index.json for faster skill registration, generated by scripts/build-skill-index.mjs
- Lazy module loading: `spectra install --lazy`, `spectra modules list`, `spectra modules add <modules>`
- Validation framework: validate-spectra.py (2,040 checks across 4 severity tiers — 0 failures, 0 warnings)
...
- RTK: spectra-appsec-assessment (7 steps — authorized AppSec/API review for surface mapping, authentication/session review, authorization and business logic, input/API risk, remediation, and handoff)
- SOC: spectra-identity-detection-review (7 steps — AD/Entra/Okta/IAM/OAuth defensive review, privilege paths, MFA/session risk, persistence surfaces, detection coverage, and response readiness)
- SOC: spectra-telemetry-readiness (7 steps — log-source coverage, parser/schema quality, retention/integrity, Blue Live Adapter fit, and detection gap mapping)
- IRT: spectra-cloud-incident-response (7 steps — cloud incident intake, evidence preservation, blast radius, containment planning, recovery, Counsel/Signal gates, and reporting)
- GRC: spectra-privacy-breach-assessment (7 steps — data exposure, jurisdiction/clock map, legal hold, notification decision support, remediation, and privacy governance report)
- Core: spectra-duel-adjudication (7 steps — ledger integrity, timeline correlation, scoring, fairness review, gap analysis, and final Referee scorecard)

- IRT: spectra-threat-intel-workflow (8 steps, ~4,421 lines — structured threat intelligence production: intelligence requirements (PIRs/SIRs, Admiralty Scale), collection (7 source categories), threat actor profiling (Diamond Model attribution), Kill Chain & ATT&CK mapping (Navigator layer), intelligence assessment (ACH, red hat, confidence calibration), IOC packaging (STIX 2.1, Sigma/YARA/Suricata), dissemination (3-tier products)). **IRT MODULE COMPLETE (5/5 workflows).**
...
- GRC: spectra-policy-lifecycle (7 steps, ~4,267 lines — complete policy lifecycle: requirement & scope (policy/standard/procedure/guideline hierarchy), research & benchmarking, drafting (RFC 2119), stakeholder review (5 categories), approval & publication, enforcement & exceptions, review & maintenance). **GRC MODULE COMPLETE (4/4 workflows). OT MODULE COMPLETE (1/1 workflow: spectra-ot-assessment — Purdue model, ICS protocol exposure, ATT&CK for ICS, IEC 62443, assessment-only).**
