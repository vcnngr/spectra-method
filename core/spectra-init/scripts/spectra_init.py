...
#!/usr/bin/env python3
"""
SPECTRA Init -- Project configuration bootstrap and config loader.

Security Protocol Engineering for Cyber Threat Response & Assessment.

Config files (flat YAML per module):
  - _spectra/core/config.yaml (core settings -- user_name, language, output_folder, etc.)
  - _spectra/{module}/config.yaml (module settings + core values merged in)

Usage:
  # Fast path -- load all vars for a module (includes core vars)
  python spectra_init.py load --module rtk --all --project-root /path

  # Load specific vars with optional defaults
  python spectra_init.py load --module rtk --vars var1:default1,var2 --project-root /path

  # Load core only
  python spectra_init.py load --all --project-root /path

  # Check if init is needed
  python spectra_init.py check --project-root /path
  python spectra_init.py check --module rtk --skill-path /path/to/skill --project-root /path

  # Resolve module defaults given core answers
  python spectra_init.py resolve-defaults --module rtk --core-answers '{"output_folder":"..."}' --project-root /path

  # Write config from answered questions
  python spectra_init.py write --answers '{"core": {...}, "rtk": {...}}' --project-root /path
"""
...
def find_project_root(llm_provided=None):
    """
    Find project root by looking for _spectra folder.
    ...
    if llm_provided:
        ...
        if (candidate / '_spectra').exists():
            return candidate
        # First run -- _spectra won't exist yet but LLM path is still valid
        if candidate.is_dir():
            return candidate
    ...
    for start_dir in [Path.cwd(), Path(__file__).resolve().parent]:
        ...
        while current_dir != current_dir.parent:
            if (current_dir / '_spectra').exists():
                return current_dir
...
def find_target_module_yaml(module_code, project_root, skill_path=None):
    ...
    Search order:
      ...
      3. _spectra/{module_code}/module.yaml (installed module location)
    """
    search_paths = []
    ...
    if project_root and module_code:
        search_paths.append(Path(project_root) / '_spectra' / module_code / 'module.yaml')
...
def load_module_config(module_code, project_root):
    """Load config for a specific module from _spectra/{module}/config.yaml."""
    config_path = Path(project_root) / '_spectra' / module_code / 'config.yaml'
...
def expand_template(value, context):
    ...
    Supports: {project-root}, {value}, {output_folder}, {var_name}, etc.
    """
    if not value or not isinstance(value, str):
        return value
    result = value
...
def apply_result_template(var_def, raw_value, context):
    ...
    E.g., result: "{project-root}/{value}" with value="_spectra-output"
    becomes "/Users/foo/project/_spectra-output"
    """
...
def cmd_load(args):
    ...
    if not project_root:
        print(json.dumps({'error': 'Project root not found (_spectra folder not detected)'}),
              file=sys.stderr)
        sys.exit(1)
    ...
    if args.all or not args.vars:
        # Default to returning all vars when neither --vars nor --all specified
...
def cmd_write(args):
    ...
    if core_answers_raw and core_def:
        ...
        # Write core config
        core_dir = project_root / '_spectra' / 'core'
    ...
    for module_code, module_answers_raw in answers.items():
        ...
        # Write module config
        module_dir = project_root / '_spectra' / module_code
...
def _write_config_file(path, data, module_label):
    ...
    with open(path, 'w', encoding='utf-8') as f:
        ...
        f.write(f'# Generated by spectra-init\n')
...
def main():
    parser = argparse.ArgumentParser(
        description='SPECTRA Init -- Project configuration bootstrap and config loader.'
    )
